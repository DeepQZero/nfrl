...
log_reg2 = SGDClassifier(loss='log')
log_reg2.fit(X, y)
# print(X[0])
# for i in range(25):
#     print(log_reg.predict_proba(X[i].reshape(1, -1)))
...
class Env:
    ...
    def get_nfl(self):
        ...
        the_play = the_state.reshape(1, -1)
        transformed_play = scaler2.transform(the_play)
        probs = log_reg2.predict_proba(transformed_play)
        play = np.random.choice([i for i in range(200)], p=list(probs[0]))
        return play

    def one(self, off_action, def_action):
        the_play = np.concatenate(
            [self.state, [off_action], [def_action]]).reshape(1, -1)
        ...
        # yardage = int(lin_reg.predict(the_play)[0])
        # print(yardage)
        time = self.state[5]
        ...
        new_state = np.array(
            [quarter, off_score, def_score, down, first_down, time])
    ...
    def step(self, off_action, def_action):
        off1, defen1 = self.one(off_action, def_action)
        nfl_off_action = self.get_nfl()
        off2, defen2 = self.one(nfl_off_action, def_action)
        self.state = self.get_new_state()
        return self.state, off1, defen1, off2, defen2
...
for i in range(100000):
    if i % 10 == 0:
        if i > 1000:
            ...
            avg_rew2 = float(np.mean(nfl_rewards[-1000:]))
            print(i, str(round(epsilon, 2)), str(round(avg_rew, 3)), str(round(avg_rew2, 3)))
    def_act = def_agent.act(state, epsilon)
    off_act = off_agent.act(state, epsilon)
    # off_act = 10
    new_state, off, defen, off2, defen2 = env.step(off_act, def_act)
    def_agent.train(state, def_act, defen)
    off_agent.train(state, off_act, off)
    rewards.append(defen)
    nfl_rewards.append(defen2)
